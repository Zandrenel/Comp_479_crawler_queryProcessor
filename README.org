...
** Running the server
source ./bin/activate
uwsgi --http 127.0.0.1:8000 --master -p 4 -w server:app
** Docker running
sudo docker build -t engine_image .
sudo docker run --network host -p 8000:8000 engine_image
or
sudo docker -d run --network host -p 8000:8000 engine_image
